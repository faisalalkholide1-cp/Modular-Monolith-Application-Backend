...
namespace AbpSolution5.OpenIddict;
...
public class OpenIddictDataSeedContributor : OpenIddictDataSeedContributorBase, IDataSeedContributor, ITransientDependency
{
    ...
    private async Task CreateScopesAsync()
    {
        ...
        {
            Name = "AbpSolution5", 
            DisplayName = "AbpSolution5 API", 
            Resources = { "AbpSolution5" }
        });
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        var appClientId = configurationSection["AbpSolution5_App:ClientId"];
        if (!appClientId.IsNullOrWhiteSpace())
        {
            var appClientRootUrl = configurationSection["AbpSolution5_App:RootUrl"]?.TrimEnd('/');
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["AbpSolution5_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["AbpSolution5_Swagger:RootUrl"]?.TrimEnd('/');
